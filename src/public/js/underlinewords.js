...
const TextProcessor = (() => {
    ...
    function updateAnchorsList() {
        const $text_container = $(parent.document).find('#text');
        $all_anchors = $text_container.find('a'); // all <a> inside #text
    }

    function removeAllHighlighted() {
        $all_anchors.filter('.highlighted').removeClass('highlighted');
    }

    function addHighlightToSelection(start_index, current_index) {
        const min = Math.min(start_index, current_index);
        const max = Math.max(start_index, current_index);
        for (let i = min; i <= max; i++) {
            $all_anchors.eq(i).addClass('highlighted');
        }
    }
    ...
    function getHighlightedTextObj(start_index, current_index) {
        const min = Math.min(start_index, current_index);
        const max = Math.max(start_index, current_index);
        let sel_array = [];
        for (let i = min; i <= max; i++) {
            sel_array.push($all_anchors.eq(i)[0]);
        }
