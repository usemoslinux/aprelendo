...
class Card
{
    ...
    public function getExampleSentencesForWord(string $word): array {
        ...
        try {
            ...
            return $this->array_unique_multidimensional($result); // avoid returning duplicate example sentences
        } catch (\PDOException $e) {
            throw new \Exception('There was an unexpected error trying to execute SQL query.');
        } finally {
            $stmt = null;
     ...
     * @return array
     */
    private function array_unique_multidimensional(array $input): array {
